...
class ModuleCreator {
  ...
  fun createModule(
      moduleName: String,
      language: com.axiom.rv2ide.fragments.sidebar.SubModuleFragment.ModuleLanguage,
      projectRoot: File,
  ): CreationResult {
    return try {
  ...
  private fun createModuleStructure(
    ...
    val javaDir =
        File(
            srcMainDir,
            if (
                language == com.axiom.rv2ide.fragments.sidebar.SubModuleFragment.ModuleLanguage.KOTLIN
            )
                "kotlin"
            else "java",
        )
  ...
  private fun generateKotlinDslBuildScript(
    ...
    val kotlinPlugin =
        if (language == com.axiom.rv2ide.fragments.sidebar.SubModuleFragment.ModuleLanguage.KOTLIN) {
          "id(\"kotlin-android\")"
        } else {
